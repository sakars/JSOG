...
//   /// @details A json shallow object means that this resource only really
//   cares
//   /// about the value of this current json object, not it's children (if
//   /// applicable) If this object is determined to be an evaluatable schema,
//   the
...
//     normalizeString to ensure that. std::string getPreferredIdentifier()
//     const
//     {
...
//     // /// @brief An instance of a schema object that is generated from the
//     json
//     // /// file.
//     // /// @note This schema object may be empty if the index hasn't
//     determined
//     // /// that this is indeed a schema that should have it's object
//     generated.
//     // std::unique_ptr<Schema> schema;
//   };

// private:
//   /// @brief Main map of resources that holds URI keys that map to the
//   shallow
//   /// json reference resource.
//   std::map<UriWrapper, std::shared_ptr<Resource>> resources_;
//   /// @brief This set of pointers mark which resources should be built, but
//   yet
//   /// have not been.
//   std::set<std::shared_ptr<Resource>> resourcesRequiringBuilding_;
//   /// @brief This set of pointers mark which resources have been built.
//   std::set<std::shared_ptr<Resource>> resourcesBuilt_;
//   std::vector<std::unique_ptr<Schema>> schemas_;
//   std::map<UriWrapper, std::reference_wrapper<Schema>> schemaMap_;

//   SetMap<UriWrapper, UnresolvedSchema> unresolvedSchemas_;

//   /// @brief Normalizes a uri string to ensure best possible map key access.
//   /// @details Non-normalized uris may be equivalent, but not be recognized
//   as
//   /// equivalent in a map. This method normalizes strings to a canonical
//   form,
//   /// so equivalent URIs are resolved to the same map.
//   static UriWrapper
//   normalizeUri(const UriWrapper &uri)
//   {
//     UriWrapper uri_(uri);
...
//                    std::string baseUri, JSONPointer jsonPointer =
//                    JSONPointer(), std::optional<Draft> draft = std::nullopt)
//   {
//     // TODO: Draft recognition is a bit iffy at this stage, consider doing
//     this
...
//     // Since schemas can only be objects or booleans, we can safely assume
//     that
//     // no $ref will reference other types, since that would be an invalid
//     JSON
...
//       }
//       addResource(std::cref(json), refs, baseUri_, jsonPointer / navKey,
//       draft);
//     };
...
//   {
//     return resources_.contains(normalizeUri(uri));
//   }

//   std::shared_ptr<Resource> &getResource(const UriWrapper &uri)
//   {
//     auto uri_ = normalizeUri(uri);
//     if (!resources_.contains(uri_))
//     {
//       throw std::runtime_error(
//           std::format("Resource {} not found in index",
//           uri_.toString().value_or("INVALID_URI")));
//     }
//     return resources_[uri_];
//   }

//   const std::shared_ptr<Resource> &getResource(const UriWrapper &uri) const
//   {
//     UriWrapper uri_ = normalizeUri(uri);
//     return resources_.at(uri_);
//   }

//   void setResource(const std::string &uri, std::shared_ptr<Resource>
//   resource)
//   {
//     resources_[normalizeUri(uri)] = resource;
//   }
...
//     if (resourcesBuilt_.contains(resource))
//     {
//       return;
//     }
//     resourcesRequiringBuilding_.insert(resource);
//   }

//   void build()
//   {
//     while (!resourcesRequiringBuilding_.empty())
//     {
//       auto resource = *resourcesRequiringBuilding_.begin();
//       auto schema = initializeSchema(resource->json, resource->baseUri,
//                                      resource->jsonPointer,
//                                      identifySchemaDraft(resource->json));
//       for (auto &ref : resource->refs)
//       {
//         schemaMap_.insert({ref, std::ref(*schema)});
//       }
//       resourcesRequiringBuilding_.erase(resource);
//       resourcesBuilt_.insert(resource);
...
//       return std::nullopt;
//     }
//     if (!schemaMap_.contains(fullUri))
//     {
//       return std::nullopt;
//     }
//     return schemaMap_.at(fullUri);
//   }

// public:
//   /// @brief Calls resolveReferences on all schemas, giving a definitive
//   method
...
//   /// @brief Extracts all schemas from the index and resets the index to a
//   clean state.
//   /// @return A vector of unique pointers to the schemas that were extracted.
//   std::vector<std::unique_ptr<Schema>> extractSchemas()
//   {
//     auto schemas = std::move(this->schemas_);
//     this->schemas_.clear();
//     resources_.clear();
//     resourcesBuilt_.clear();
//     resourcesRequiringBuilding_.clear();
//     schemaMap_.clear();
//     return schemas;
//   }
...
//   // std::set<std::shared_ptr<Resource>> getRequiredResources(const Resource
//   &resource) const
//   // {
...
//   //   {
//   //     const auto reqIdentifier =
//   reqResource->schema->getIdentifier().value();
...
//   //   declaration += "std::optional<" + resource.schema->getTypeName() + ">
...
//   // void generateResources(std::filesystem::path srcDir,
//   std::optional<std::filesystem::path> includeDir = std::nullopt) const
//   // {
...
//   //   }
//   //   for (const auto &resource : resourcesBuilt_)
//   //   {
//   //     if (resource->schema == nullptr)
//   //     {
//   //       continue;
