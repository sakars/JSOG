...
// /// @brief The main class that oversees the generation of schemas from raw
// /// documents to output C++ files.
...
//   void addDocument(Document &&doc) { documents.emplace_back(std::move(doc));
...
//       index.addResource(doc.json_, {doc.fileUri_.toString().value_or("")},
//                         doc.fileUri_.toString().value_or(""));
//     }
//   }
...
//   static void
//   extractSchemasFromIndex(ResourceIndex &index,
//                           std::vector<std::unique_ptr<Schema>> &schemas) {
//     schemas = std::move(index.extractSchemas());
//   }

// public:
//   std::map<std::filesystem::path, std::string> generateSchemas(
//       std::filesystem::path srcDir,
//       std::optional<std::filesystem::path> includeDir = std::nullopt) {
//     // Setup
//     ResourceIndex index;
//     std::vector<std::unique_ptr<Schema>> schemas;
