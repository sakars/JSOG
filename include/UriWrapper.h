...
public:
  ...
  /// @param relative Relative URI to apply to the base URI as defined by RFC3986
  /// @return A new URI instance that is the result of applying the relative URI
  ...
  /// @brief Performs Syntax-Based Normalization on the URI according to RFC3986 Section 6.2.2
  /// @details This function normalizes the URI according to the rules defined in RFC3986 Section 6.2.2
  /// @note This function does not perform scheme-specific normalization such as retaining case sensitivity should the scheme be case-sensitive
  void normalize();

  /// @brief Attempts to convert the URI to a string representation.
  /// @return The string representation of the URI, or std::nullopt if the URI is invalid
  std::optional<std::string> toString() const noexcept;

  /// @brief Attempts to convert the URI to a string representation without the fragment.
  /// @return The string representation of the URI without the fragment, or std::nullopt if the URI is invalid
  /// @note Not only does this function remove the fragment, but it also removes the octothorpe (#) character that precedes the fragment
  std::optional<std::string> toFragmentlessString() const;

  /// @brief Attempts to retrieve the fragment of the URI
  /// @return The fragment of the URI if it exists, or std::nullopt if the URI is invalid or has no fragment
  /// @note The fragment is the part of the URI that follows the octothorpe (#) character
  /// This function does not return the string with the octothorpe character
  ...
  /// @param hasStartingOctothorpe Whether the provided fragment has a starting octothorpe (#) character
