...
IndexedSyncedSchema interpretDraft07IdentifiableSchema(
  ...
  if (json.contains("$ref")) {
    indexedSyncedSchema.reinterpretables_.ref_ =
  ...
  if (json.contains("multipleOf")) {
    indexedSyncedSchema.numberProperties_.multipleOf_ =
        json.at("multipleOf").get<double>();
  }

  if (json.contains("maximum")) {
    indexedSyncedSchema.numberProperties_.maximum_ =
        json.at("maximum").get<double>();
  }

  if (json.contains("exclusiveMaximum")) {
    indexedSyncedSchema.numberProperties_.exclusiveMaximum_ =
        json.at("exclusiveMaximum").get<double>();
  }

  if (json.contains("minimum")) {
    indexedSyncedSchema.numberProperties_.minimum_ =
        json.at("minimum").get<double>();
  }

  if (json.contains("exclusiveMinimum")) {
    indexedSyncedSchema.numberProperties_.exclusiveMinimum_ =
        json.at("exclusiveMinimum").get<double>();
  }

  if (json.contains("maxLength")) {
    indexedSyncedSchema.stringProperties_.maxLength_ =
        json.at("maxLength").get<size_t>();
  }

  if (json.contains("minLength")) {
    indexedSyncedSchema.stringProperties_.minLength_ =
        json.at("minLength").get<size_t>();
  }

  if (json.contains("pattern")) {
    indexedSyncedSchema.stringProperties_.pattern_ =
  ...
  if (json.contains("items")) {
    if (json.at("items").is_object()) {
      ...
      indexedSyncedSchema.arrayProperties_.items_ =
          relUriToDependency(itemsPointer.toFragment());
    } else if (json.at("items").is_array()) {
      indexedSyncedSchema.arrayProperties_.tupleableItems_ =
          std::vector<size_t>();
      for (size_t i = 0; i < json.at("items").size(); i++) {
        ...
        // indexedSyncedSchema.arrayProperties_.tupleableItems_->push_back(
      ...
      if (json.contains("additionalItems")) {
        indexedSyncedSchema.arrayProperties_.items_ = relUriToDependency(
  ...
  if (json.contains("maxItems")) {
    indexedSyncedSchema.arrayProperties_.maxItems_ =
        json.at("maxItems").get<size_t>();
  }

  if (json.contains("minItems")) {
    indexedSyncedSchema.arrayProperties_.minItems_ =
        json.at("minItems").get<size_t>();
  }

  if (json.contains("uniqueItems")) {
    indexedSyncedSchema.arrayProperties_.uniqueItems_ =
        json.at("uniqueItems").get<bool>();
  }

  if (json.contains("contains")) {
    JSONPointer containsPointer = identifiableSchema.pointer_ / "contains";
    indexedSyncedSchema.arrayProperties_.contains_ =
        relUriToDependency(containsPointer.toFragment());
  }

  if (json.contains("maxProperties")) {
    indexedSyncedSchema.objectProperties_.maxProperties_ =
        json.at("maxProperties").get<size_t>();
  }

  if (json.contains("minProperties")) {
    indexedSyncedSchema.objectProperties_.minProperties_ =
        json.at("minProperties").get<size_t>();
  }

  if (json.contains("required")) {
    indexedSyncedSchema.objectProperties_.required_ = std::set<std::string>();
    for (const auto& item : json.at("required")) {
      indexedSyncedSchema.objectProperties_.required_->insert(
          item.get<std::string>());
    }
  }

  if (json.contains("properties")) {
    indexedSyncedSchema.objectProperties_.properties_ =
    ...
    for (const auto& [key, value] : json.at("properties").items()) {
      ...
      indexedSyncedSchema.objectProperties_.properties_.emplace(
          key, relUriToDependency(propertyPointer.toFragment()));
    }
  }

  if (json.contains("patternProperties")) {
    indexedSyncedSchema.objectProperties_.patternProperties_ =
        std::map<std::string, size_t>();
    for (const auto& [key, value] : json.at("patternProperties").items()) {
      ...
      indexedSyncedSchema.objectProperties_.patternProperties_->emplace(
  ...
  if (json.contains("additionalProperties")) {
    ...
    indexedSyncedSchema.objectProperties_.additionalProperties_ =
        relUriToDependency(additionalPropertiesPointer.toFragment());
  }

  if (json.contains("dependencies")) {
    indexedSyncedSchema.objectProperties_.propertyDependencies_ =
        std::map<std::string, std::vector<std::string>>();
    indexedSyncedSchema.objectProperties_.schemaDependencies_ =
        std::map<std::string, size_t>();
    for (const auto& [key, value] : json.at("dependencies").items()) {
      if (value.is_array()) {
        ...
        indexedSyncedSchema.objectProperties_.propertyDependencies_->emplace(
            key, dependencies);
      } else {
        JSONPointer dependencyPointer =
            identifiableSchema.pointer_ / "dependencies" / key;
        indexedSyncedSchema.objectProperties_.schemaDependencies_->emplace(
  ...
  if (json.contains("propertyNames")) {
    ...
    indexedSyncedSchema.objectProperties_.propertyNames_ =
        relUriToDependency(propertyNamesPointer.toFragment());
  }

  if (json.contains("if")) {
    JSONPointer ifPointer = identifiableSchema.pointer_ / "if";
    indexedSyncedSchema.reinterpretables_.if_ =
        relUriToDependency(ifPointer.toFragment());
  }

  if (json.contains("then")) {
    JSONPointer thenPointer = identifiableSchema.pointer_ / "then";
    indexedSyncedSchema.reinterpretables_.then_ =
        relUriToDependency(thenPointer.toFragment());
  }

  if (json.contains("else")) {
    JSONPointer elsePointer = identifiableSchema.pointer_ / "else";
    indexedSyncedSchema.reinterpretables_.else_ =
        relUriToDependency(elsePointer.toFragment());
  }

  if (json.contains("allOf")) {
    indexedSyncedSchema.reinterpretables_.allOf_ = std::vector<size_t>();
    for (size_t i = 0; i < json.at("allOf").size(); i++) {
      JSONPointer allOfPointer =
          identifiableSchema.pointer_ / "allOf" / std::to_string(i);
      indexedSyncedSchema.reinterpretables_.allOf_->push_back(
          relUriToDependency(allOfPointer.toFragment()));
    }
  }

  if (json.contains("anyOf")) {
    indexedSyncedSchema.reinterpretables_.anyOf_ = std::vector<size_t>();
    for (size_t i = 0; i < json.at("anyOf").size(); i++) {
      JSONPointer anyOfPointer =
          identifiableSchema.pointer_ / "anyOf" / std::to_string(i);
      indexedSyncedSchema.reinterpretables_.anyOf_->push_back(
          relUriToDependency(anyOfPointer.toFragment()));
    }
  }

  if (json.contains("oneOf")) {
    indexedSyncedSchema.reinterpretables_.oneOf_ = std::vector<size_t>();
    for (size_t i = 0; i < json.at("oneOf").size(); i++) {
      JSONPointer oneOfPointer =
          identifiableSchema.pointer_ / "oneOf" / std::to_string(i);
      indexedSyncedSchema.reinterpretables_.oneOf_->push_back(
  ...
  if (json.contains("not")) {
    ...
    indexedSyncedSchema.reinterpretables_.not_ =
        relUriToDependency(notPointer.toFragment());
  }
